...
const createRequest = (
  ...
  const extendArgs = payloadArgs || [];

  axiosUrl = extendUrl(axiosUrl, ...extendArgs);

  return new Promise((resolve, reject) => {
    const method = () => {
      const result = ['patch', 'put', 'post', 'delete'].includes(methodName)
        ? axios[methodName](axiosUrl, { ...payloadData }, payloadOptions || {})
        : axios[methodName](axiosUrl, payloadOptions || {});
      return result;
    };
    method()
      .then((response) => {
